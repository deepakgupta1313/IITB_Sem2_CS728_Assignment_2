...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm)
{
  ...
  else if(sparm->loss_function == 2) /* type 1 loss: constant penalty per wrong POS tag */
  {
	 unsigned int penalty = 0;
	 const unsigned int minSize = min(y.getLength(), ybar.getLength());
	  for(unsigned int i = 0; i < minSize; i++)
	  	if(ybar.getTag(i) != y.getTag(i)) penalty++;
	 double rett=sqrt((double)penalty);
	 return rett;
	  //return (double)penalty;
  }
  else if(sparm->loss_function == 3) /* type 1 loss: constant penalty per wrong POS tag */
  {
	 unsigned int penalty = 0;
	 const unsigned int minSize = min(y.getLength(), ybar.getLength());
	  for(unsigned int i = 0; i < minSize; i++)
	  	if(ybar.getTag(i) != y.getTag(i)) penalty++;
	  double rett=log((double)penalty+1.0);
	 return rett;
	  //return (double)penalty;
  }
  else if(sparm->loss_function == 4) /* type 1 loss: constant penalty per wrong POS tag */
  {
	 unsigned int penalty = 0;
	 const unsigned int minSize = min(y.getLength(), ybar.getLength());
	  for(unsigned int i = 0; i < minSize; i++)
	  	if(ybar.getTag(i) != y.getTag(i)) penalty++;
	  double rett=log((double)penalty+1.0)+sqrt((double)penalty);
	 return rett;
  }
  else
  {
